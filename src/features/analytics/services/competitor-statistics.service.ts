...
export const competitorStatisticsService = {
  compute(
    ...
    vehicles: CompetitorVehicle[],
  ): CompetitorStatistics {
    return {
      totalCompetitors: competitors.length,
      totalVehicles: vehicles.length,
      byCompetitor: buildRanking(
        vehicles.map((v) => ({ key: v.competitor_name, price: v.price })),
      ),
      byBrand: buildRanking(
        vehicles.map((v) => ({ key: v.brand, price: v.price })),
      ),
